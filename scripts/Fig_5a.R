...
head(re_allgene0)
# write.xlsx(re_allgene0,"gsea/gsea_allgene_sig_go_term.xlsx")
...
# write.xlsx(re_allgene,paste0("gsea/gsea_allgene_results_",date_log,".xlsx"))
...
head(re_samplesize);length(unique(re_samplesize$species)) #16
...
re_allgene <- re_allgene %>% 
  mutate(padj_annot=case_when(
    ...
    padj_annot2=case_when(
      padj>=0.05&padj<0.1 ~ "#",
      TRUE ~ ""),
    padj_annot3=case_when(
      padj<0.05 ~ "+" , # #, †, §
      TRUE ~ "")
    )
table(re_allgene$padj_annot) 
table(re_allgene$padj_annot1)
table(re_allgene$padj_annot2)
table(re_allgene$padj_annot3)
...
p_bub <- ggplot(re_allgene,
  ...
  geom_text(aes(label=padj_annot3),color="black",size=5,nudge_y = 0.08)+
...
egg::ggarrange(
  p_t2d_status,blank,p_gene_num,p_box,p_bub, 
  blank,blank,blank,blank,p_cat,
  nrow = 2,ncol = 5,
  heights =  c(5,0.1),
  widths = c(0.06,-0.01,1,1,6)) 
dev.off()
