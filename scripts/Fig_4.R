...
fig5_path <- "/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/dropbox/fig5/"
...
gdata::keep(spe_t2d_status,sure = T)
...
re_pglmm_t$elpd_2se <- re_pglmm_t$elpd_diff + 2*re_pglmm_t$se_diff
...
p_pglmm_metf <- 
  ...
  geom_text(aes(x=(-(elpd_diff-se_diff)+1),label=sig,group=model_metf),
            position = position_dodge(0.8),color="black",vjust=0.8)+
  theme_bw()+
...
p_pglmm_metf
# ggsave(paste0(fig5_path,"pglmm_metf_forest_plot.png"),
...
head(re_t2d_plot);dim(re_t2d_plot) 
...
pdf(paste0(fig5_path,"fig_5a_",date_log,".pdf"), 
...
pdf(paste0(fig5_path,"species_t2d_status_legend.pdf"),
...
pdf(paste0(fig5_path,"sample_size_legend.pdf"),
    width = 5,height = 5,onefile = F)
