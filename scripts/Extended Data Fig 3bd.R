setwd("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/figS5")
...
spe_all <- Reduce(union, list(
  meta_0[meta_0$qval.fdr<0.1,]$feature,
  meta_1[meta_1$qval.fdr<0.1,]$feature,
  meta_2[meta_2$qval.fdr<0.1,]$feature,
  meta_3[meta_3$qval.fdr<0.1,]$feature,
  meta_4[meta_4$qval.fdr<0.1,]$feature
))
...
# -1.40759 -0.36187  0.12221  0.06819  0.47043  1.47559

sum(heatmap$coef>1.4) # 2
heatmap$coef[heatmap$coef>1.4] <- 1.4
sum(heatmap$coef<(-1.4)) # 1
heatmap$coef[heatmap$coef<(-1.4)] <- (-1.4)
...
table(heatmap[heatmap$qval.fdr<0.1,]$model)
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/figS5/fig_s5b.pdf",
    width = 7.8, height = 24, onefile = F) # Open a new pdf file
egg::ggarrange(heat1, heat1_0, heat2, heat2_0, nrow=2, ncol=2,
               widths = c(1, 0.1), heights=c(24,38))
dev.off() # Close the file

cor(beta$mod0, beta$mod1, method="spearman")
# scatter plot
p1 <- ggplot(aes(x=mod0, y=mod1), data=beta) +
  geom_point() +
  geom_hline(yintercept = 0, col="red") +
  geom_vline(xintercept = 0, col="red") +
  scale_x_continuous(breaks=c(-1.4,-0.7,0,0.7,1.4), limits=c(-1.45,1.5)) +
  scale_y_continuous(breaks=c(-1,-0.5,0,0.5,1.0,1.5), limits=c(-1.14, 1.5)) +
  ...
  annotate(geom="text", x=-1.0, y=1.3, col="black", label="r=0.96", size=4.2)

cor(beta$mod3, beta$mod1, method="spearman")
p3 <- ggplot(aes(x=mod1, y=mod3), data=beta) +
  geom_point() +
  geom_hline(yintercept = 0, col="red") +
  geom_vline(xintercept = 0, col="red") +
  scale_x_continuous(breaks=c(-1,-0.5,0,0.5,1.0,1.5), limits=c(-1.14, 1.5)) +
  scale_y_continuous(breaks=c(-0.8,-0.4,0,0.4,0.8,1.2), limits=c(-0.9,1.3)) +
  labs(x="Adjustment for\nage + sex + BMI",
       y="Adjustment for\nage + sex + BMI + metf",
       title="") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.title = element_text(colour = "black", size=12),
        panel.border = element_rect(colour = "black", size=1.2)) +
  annotate(geom="text", x=-0.7, y=1.1, col="black", label="r=0.86", size=4.2)

pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/figS5/fig_s5d.pdf",
