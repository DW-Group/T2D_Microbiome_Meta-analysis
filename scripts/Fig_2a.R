...
spe_all <- Reduce(union, list(
  ...
  meta_2trend[meta_2trend$qval.fdr<0.25,]$feature,
...
# -0.47354 -0.16865  0.04027  0.03577  0.21554  0.71066

sum(heatmap_trend$coef>0.7) # 1
heatmap_trend$coef[heatmap_trend$coef>=0.7] <- 0.7

sum(data1trend$coef>0) # 50
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R1/fig2/figure2a.pdf",
    width = 7.5, height = 36, onefile = F) # Open a new pdf file
egg::ggarrange(heat1_trend, heat1_0, 
               heat2_trend, heat2_0, nrow=2, ncol=2,
               widths = c(1,0.2), heights=c(50,48))
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R1/fig2/figure2a_legend.pdf",
    width = 3, height = 2, onefile = F) # Open a new pdf file
grid::grid.draw(ggpubr::get_legend(trendlegend))
dev.off() # Close the file

save.image(file="/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R1/fig2/figure2a.RData")
