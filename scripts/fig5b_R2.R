...
genes_del_v5 <- paste0("UniRef90_",genes_del_v5)
head(genes_del_v5)
...
head(anpan_re2);dim(anpan_re2) #105118/14
...
source("~/my_code/t2d_meta/anpan/3.anpan.gene.model/visualization2.R")
p <- plot_results(res = anpan_re2,
                   model_input = model_input3,
...
p[[color_bar_i]]
(aes_name = p[[color_bar_i]]$scales$scales[[study_i]]$aesthetics)
(guide = p[[color_bar_i]]$scales$scales[[study_i]]$guide)


guide$available_aes = aes_name
p[[color_bar_i]]$scales$scales[[study_i]] <- 
...
diff_pre <- 100
pdf(paste0("anpan_1_by_1/heatmap_bmi_",bugName,
           "_samp_",perct_samp,
           "_beta_",beta_thresh,
           "_manual.filter_",
           date_log,
...
pdf("anpan_1_by_1/heatmap_study_legend.pdf",width = 12,height = 3)
p[[color_bar_i]]
dev.off()
...
dim(gene_level_df) #218/14
write.xlsx(gene_level_df,"genes_heatmap.xlsx")
...
### 2.heatmap tile ---------
# plot_data = as.data.table(anpan_re2)[anno[plot_data, on = 'gene'], on = 'gene']
...
table(unique(plot_data$Category) %in% names(col_panel))
...
### 4.t value bar -----
head(plot_data)
summary(plot_data$statistic)
table(abs(plot_data$statistic)>4)
library(RColorBrewer)
breaklist <- seq(-6,6,by=0.001)
red_blue <- rev(brewer.pal(n=11,name="RdBu"))
scales::show_col(red_blue)
col_pal <- colorRampPalette(red_blue)(length(breaklist))

t_bar <- ggplot(plot_data,aes(x=1,y=gene))+
  geom_tile(aes(fill=statistic),width=1)+ #color=category,
  # geom_tile(aes(fill=log10(p.value)),width=1)+ #color=category,
  # labs(fill="log10(P)")+ #,color="Category"
  labs(fill="t-statistics")+ #,color="Category"
  # scale_y_discrete(breaks = lab_df$gene, position = 'right')  +
  # scale_fill_viridis_c(option = "plasma") +
  # scale_fill_gradientn(colors = rev(paletteer::paletteer_c("ggthemes::Blue",60))) +
  scale_fill_gradientn(colors = (col_pal),limits=c(-6,6)) + #rev
  coord_cartesian(expand = FALSE)+
  theme_void()+
  theme(#panel.grid = element_blank(),
    #axis.text.x = element_blank(),
...
## legend
t_bar_lgd <- ggplot(plot_data,aes(x=1,y=gene))+
  geom_tile(aes(fill=statistic),width=1)+ #color=category,
  # geom_tile(aes(fill=log10(p.value)),width=1)+ #color=category,
  labs(fill="t-statistics")+ #,color="Category"
  # labs(fill="log10(P)")+ #,color="Category"
  # scale_fill_gradientn(colors = rev(paletteer::paletteer_c("ggthemes::Orange",60))) +
  scale_fill_gradientn(colors = (col_pal),limits=c(-6,6)) + #rev
  # scale_fill_viridis_c(option = "plasma") +
  # scale_x_discrete(breaks=breaks_go,labels=labels_go)+
  coord_cartesian(expand = FALSE)+
...
t_bar_legend <- cowplot::get_legend(t_bar_lgd)
pdf(paste0("anpan_1_by_1/t_bar_legend_",date_log,".pdf"), 
    width = 5, height = 5, onefile = F) # Open a new pdf file
grid::grid.draw(t_bar_legend)
dev.off()
...
design_str = "
    ...
    "
print(heatmap_tile)
p = patchwork::wrap_plots(anno_plot, heatmap_tile,t_bar,p_cat, #p_bar
                          ncol = 3,
                          guides = 'collect',
                          design = design_str
) +
