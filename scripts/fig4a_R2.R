...
select <- dplyr::select
(date_log <- Sys.Date())
fig4_path <- "/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/fig4/"
...
(re_pglmm_metf)
re_pglmm_metf$t2d_status <- 
  ifelse(is.na(re_pglmm_metf$t2d_status),"Non-significant",
         re_pglmm_metf$t2d_status)
...
## 1.T2D status -----
table(re_pglmm_metf$t2d_status)
p_t2d_status <- ggplot(re_pglmm_metf,aes(x=1,y=species_update,fill=t2d_status))+
  geom_col(width = 1)+
  scale_fill_manual(
    breaks = c("Non-significant","T2D depleted","T2D enriched"),
...
## legends ----
## t2d status
p_t2d_lgd <- ggplot(re_pglmm_metf,aes(x=1,y=species_update,fill=t2d_status))+
  geom_col(width = 1)+
  scale_fill_manual(
    breaks = c("T2D enriched","T2D depleted","Non-significant"),
