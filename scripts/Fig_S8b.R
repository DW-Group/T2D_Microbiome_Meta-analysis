...
pwy_T2D <- results_T2D %>% filter(feature %in% pwy) %>% 
  mutate(group="T2D", star=case_when(qval.fdr<0.25 ~ "*",
                                     TRUE ~ "")) %>% 
  select(group, feature, pathway, coef, pval, star) %>% arrange(desc(coef))
...
pwy_bmi <- results_bmi %>% 
  mutate(group="BMI", 
         star=case_when(fdr<0.25 ~ "*",
                        TRUE ~ "")) %>% 
  select(group, feature, pathway, coef, pval, star)
...
library(ggdendro)
library(scales)
heat_all <- ggplot(segment(pwy_all_dend_data)) + 
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/0323/biomarker/figure_pwy_v4.pdf", 
    width = 9.5, height = 6.5, onefile = F) # Open a new pdf file
egg::ggarrange(heat_all, blank, heat1_all, 
               nrow=1, ncol=3, widths=c(2,-0.8,7))
dev.off() # Close the file

save.image(file="/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/0323/biomarker/figure_pwy_v4.RData")
