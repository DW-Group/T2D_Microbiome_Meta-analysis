...
ecs_T2D <- results_T2D %>% filter(feature %in% ecs_18 & k>1) %>% 
  ...
  select(group, feature, coef, pval, star1, star2, star3) %>% arrange(desc(coef)) %>% 
...
ecs_bmi <- results_bmi %>% filter(feature %in% ecs_18) %>% 
  ...
  select(group, feature, coef, pval, star1, star2, star3)
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R1/figS8/figure_s8b.pdf", 
    width = 9.05, height = 6.55, onefile = F) # Open a new pdf file
print(heat1_all)
