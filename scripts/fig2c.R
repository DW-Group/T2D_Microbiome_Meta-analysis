...
spe00 <- data.frame(id=colnames(spe_resid[,2:8118]),
                    spe=as.numeric(t(spe_resid[spe_resid$spe=="s__Coprococcus_eutactus", -1])) + spe_means[spe_means$spe=="s__Coprococcus_eutactus",]$mean)
...
p0 <- ggplot(aes(x=status_new, y=mean), data=spe00plot) +
  ...
  labs(title="Coprococcus eutactus") +
  ylab("") +
  theme_bw() +
  ...
  annotate(geom="text", x=1.2, y=ymin+(ymax-ymin)/18, size=3.7, col="black", label="FDR=0.01")
...
spe66 <- data.frame(id=colnames(spe_resid[,2:8118]),
                    spe=as.numeric(t(spe_resid[spe_resid$spe=="s__Bacteroides_fragilis", -1])) + spe_means[spe_means$spe=="s__Parasutterella_excrementihominis",]$mean)
...
p6 <- ggplot(aes(x=status_new, y=mean), data=spe66plot) +
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.title.y = element_text(color="black", size=12),
        legend.position = "none") +
  annotate(geom="text", x=1.2, y=ymax-(ymax-ymin)/18, size=3.7, col="black", label="FDR=0.04")

pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/fig2/fig2c.pdf",
    width = 5.5, height = 5.1, onefile = F) # Open a new pdf file
ggpubr::ggarrange(p2,p0,p5,p6, nrow=2, ncol=2, widths = c(1.03,1))
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/R2/fig2/figure2c_legend.pdf",
