...
meta_3_plot <- meta_3 %>% 
  filter(feature %in% c(
    ...
    "NAGLIPASYN-PWY",
    "LACTOSECAT-PWY"
  )) %>% mutate(group=case_when(feature=="PWY-6168" ~ "Vitamin biosynthesis",
                                feature=="PWY-7357" ~ "Vitamin biosynthesis",
                                feature=="PWY-6901" ~ "Glucose homeostasis",
                                feature=="PWY-5121" ~ "Glucose homeostasis",
...
karlsson0 <- meta3$maaslin_fits$`Karlsson_2013 (SWE)` %>% filter(feature %in% pathway_0) %>% mutate(study="Karlsson_2013 (SWE)")
direct0 <- meta3$maaslin_fits$`DIRECT-PLUS (ISR)` %>% filter(feature %in% pathway_0) %>% mutate(study="DIRECT-PLUS (ISR)")
mbs0 <- meta3$maaslin_fits$`NHSII (USA)` %>% filter(feature %in% pathway_0) %>% mutate(study="NHSII (USA)")
mlvs0 <- meta3$maaslin_fits$`HPFS (USA)` %>% filter(feature %in% pathway_0) %>% mutate(study="HPFS (USA)")
qin0 <- meta3$maaslin_fits$`Qin_2012 (CHN)` %>% filter(feature %in% pathway_0) %>% mutate(study="Qin_2012 (CHN)")
zhong0 <- meta3$maaslin_fits$`Zhong_2019 (CHN)` %>% filter(feature %in% pathway_0) %>% mutate(study="Zhong_2019 (CHN)")
sol0 <- meta3$maaslin_fits$`SOL (USA)` %>% filter(feature %in% pathway_0) %>% mutate(study="SOL (USA)")
pedersen0 <- meta3$maaslin_fits$`Fromentin_2022 (DEU_DNK_FRA)` %>% filter(feature %in% pathway_0) %>% mutate(study="Fromentin_2022 (DEU/DNK/FRA)")
wua0 <- meta3$maaslin_fits$`Wu_2020a (SWE)` %>% filter(feature %in% pathway_0) %>% mutate(study="Wu_2020a (SWE)")
wub0 <- meta3$maaslin_fits$`Wu_2020b (SWE)` %>% filter(feature %in% pathway_0) %>% mutate(study="Wu_2020b (SWE)")
...
# -0.88428 -0.03836  0.10551  0.12117  0.25200  1.42113
sum(studies0$coef<(-0.5)) # 2
sum(studies0$coef>1.0) # 1
studies0$coef[studies0$coef>=1] <- 1
studies0$coef[studies0$coef<=(-0.5)] <- (-0.5)

summary(pathway0$lci)
# Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -0.08423  0.06784  0.10002  0.08085  0.11410  0.29526
summary(pathway0$uci)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.03632  0.15871  0.20475  0.19248  0.25793  0.46415 
...
karlsson0 <- meta3$maaslin_fits$`Karlsson_2013 (SWE)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Karlsson_2013 (SWE)")
direct0 <- meta3$maaslin_fits$`DIRECT-PLUS (ISR)` %>% filter(feature %in% enzyme_0) %>% mutate(study="DIRECT-PLUS (ISR)")
mbs0 <- meta3$maaslin_fits$`NHSII (USA)` %>% filter(feature %in% enzyme_0) %>% mutate(study="NHSII (USA)")
mlvs0 <- meta3$maaslin_fits$`HPFS (USA)` %>% filter(feature %in% enzyme_0) %>% mutate(study="HPFS (USA)")
qin0 <- meta3$maaslin_fits$`Qin_2012 (CHN)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Qin_2012 (CHN)")
zhong0 <- meta3$maaslin_fits$`Zhong_2019 (CHN)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Zhong_2019 (CHN)")
sol0 <- meta3$maaslin_fits$`SOL (USA)` %>% filter(feature %in% enzyme_0) %>% mutate(study="SOL (USA)")
pedersen0 <- meta3$maaslin_fits$`Fromentin_2022 (DEU_DNK_FRA)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Fromentin_2022 (DEU/DNK/FRA)")
wua0 <- meta3$maaslin_fits$`Wu_2020a (SWE)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Wu_2020a (SWE)")
wub0 <- meta3$maaslin_fits$`Wu_2020b (SWE)` %>% filter(feature %in% enzyme_0) %>% mutate(study="Wu_2020b (SWE)")
...
summary(studies0$coef)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.828678  0.002694  0.167408  0.182517  0.358437  1.111895
sum(studies0$coef<(-0.5)) # 3
sum(studies0$coef>1) # 2
studies0$coef[studies0$coef>=1] <- 1
studies0$coef[studies0$coef<=(-0.5)] <- (-0.5)

summary(enzyme0$lci)
# Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -0.06159  0.11628  0.15113  0.16436  0.20098  0.47915
summary(enzyme0$uci)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.0255  0.2516  0.2905  0.3230  0.4098  0.7684
...
ecs_1 <- ggplot(ecs_1_data, aes(x=coef2, y=lab, xmin=lci2, xmax=uci2)) +
  ...
  scale_x_continuous(breaks=brk2,
                     labels=c(-0.1,0,0.3,0.6,0.9), 
                     limits=c(brk2[1],brk2[5])) +
...
load("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/1106/figure3/figure3_col.RData")
rownames(col26) <- NULL
setdiff(species, col26$species)
# [1] "Alistipes putredinis"    "Collinsella aerofaciens" "Eubacterium eligens"
col27 <- col26[col26$species %in% species, ]
col27[25:27,]$species <- c("Alistipes putredinis","Collinsella aerofaciens","Eubacterium eligens")
col27[25:27,]$col <- c("#99CC00FF","#D60047FF","#6BD76BFF")
col27 <- col27 %>% arrange(species)
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/dropbox/fig4/figure4.pdf", 
    width = 19.5, height = 16.5, onefile = F) # Open a new pdf file
egg::ggarrange(pwy_0, pwy_1, pwy_2, pwy_3, 
               ecs_0, ecs_1, ecs_2, ecs_3, 
               nrow=2, ncol=4,
               widths = c(0.06,1,1,1), heights = c(1.7,1.9))
dev.off() # Close the file

# legend_spe
legend_spe <- ggplot(ecs_all_spe_v3_xx, aes(x=enzy, y=avg)) + 
  geom_col(aes(fill=species), position = position_stack(reverse = TRUE)) +
  scale_fill_manual(values=col) +
  guides(fill=guide_legend(ncol = 1)) +
  labs(fill="Species") +
  ...
  theme(axis.text.y=element_blank(),
        ...
        legend.text = element_text(colour = "black", size = 16, face = "italic"),
        legend.title = element_text(colour = "black", size = 16))

pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/dropbox/fig4/figure4_legend_spe.pdf", 
    width = 5, height = 8, onefile = F) # Open a new pdf file
grid::grid.draw(ggpubr::get_legend(legend_spe))
dev.off() # Close the file
...
pdf("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/dropbox/fig4/figure4_legend_grp.pdf", 
    width = 12, height = 3, onefile = F) # Open a new pdf file
grid::grid.draw(ggpubr::get_legend(legend_grp))
dev.off() # Close the file

save.image("/n/holylabs/LABS/dwang_lab/Lab/T2D_meta/3.analysis/dropbox/fig4/figure4.RData")
